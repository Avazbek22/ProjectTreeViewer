...
public sealed class FileSystemScanner : IFileSystemScanner
{
	...
	public ScanResult<HashSet<string>> GetExtensions(string rootPath, IgnoreRules rules, IReadOnlySet<string> allowedRootFolders)
	{
		...
		try
		{
			rootFiles = Directory.GetFiles(rootPath);
		}
		catch (UnauthorizedAccessException)
		{
		...
		try
		{
			rootDirs = Directory.GetDirectories(rootPath);
		}
		catch (UnauthorizedAccessException)
		{
			return new ScanResult<HashSet<string>>(exts, true, true);
		...
		foreach (var dir in rootDirs)
		{
			...
			if (allowedRootFolders.Count > 0 &&
				!allowedRootFolders.Contains(dirInfo.Name))
				continue;

			CollectExtensions(dir, rules, exts, ref hadAccessDenied);
		}
